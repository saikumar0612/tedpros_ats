...
describe('MembershipsComponent', () => {
  ...
  it("#ngonint", fakeAsync(() => {
    component.ngOnInit();
    tick();
    userServiceStub.getMemberships()
      .pipe(map(response => {
        component.memberships = response.json().data;
        component.filterData = component.memberships;
        component.availableRecords = component.filterData.length;
      }
      ))
  }));
  ...
  it('#search', fakeAsync(() => {
    ...
    tick();
    expect(component.filterData.length).toEqual(component.memberships.length);
