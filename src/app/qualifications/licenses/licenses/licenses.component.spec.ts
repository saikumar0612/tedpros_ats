...
describe('LicensesComponent', () => {
  ...
  it("#ngonint", fakeAsync(() => {
    component.ngOnInit();
    tick();
    userServiceStub.getLicenses()
      .pipe(map(response => {
        component.licenses = response.json().data;
        component.filterData = component.licenses;
  ...
  it('#search', fakeAsync(() => {
    ...
    tick();
    expect(component.filterData.length).toEqual(component.licenses.length);
