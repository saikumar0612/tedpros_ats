...
describe('EditCustomerComponent', () => {
  ...
  it('#getCompanyInfo', fakeAsync(() => {
    component.companypopup = true;
    component.getCompanyInfo();
    tick();
    expect(component.cusInfo.industryType).toEqual('4');
  }));
  ...
  it('#updateStatus', fakeAsync(() => {
    component.updateStatus('', contact);
    tick();
    expect(component.modelPopUp).toBe(true);
  }));
  ...
  it('#assign', fakeAsync(() => {
    component.assign();
    tick();
    expect(component.modelPopUp).toBe(false);
  }));
  ...
  it('#addContact', fakeAsync(() => {
    component.contacts = [];
    component.addContact(myFrm, '1', '', 'Req gather', '0');
    tick();
    ...
    component.addContact(myFrm, '', '', 'Req gather', '0');
    tick();
    expect(component.error).toEqual('Select any user');

    component.addContact(myFrm, '1', '', '', '0');
    tick();
  ...
  it('#editsinglestatus', fakeAsync(() => {
    component.editsinglestatus('1', 0);
    tick();
    expect(component.contacts[0].status).toEqual('1');
  }));
