...
describe('AddOfferletterComponent', () => {
  ...
  it('#ngOnInit', fakeAsync(() => {
    ...
    // const temp = component.config.on.pluginsLoaded.bind(component);
    // component.config.on.pluginsLoaded();
    // // spyOn('temp');
    // console.log(temp());
  }));

  it('#getOfferLetterTemplateById', fakeAsync(() => {
    component.getOfferLetterTemplateById('1');
    userServiceStub.getOfferLetterById('1')
      .pipe(map(response => {
        component.offerLetterTemplate = response.json().data;
      }))
    tick();
    expect(component.data.content).toEqual(component.offerLetterTemplate.template);
  }));

  it('#getUserOfferLetter', fakeAsync(() => {
    component.getUserOfferLetter();
    tick();
    userServiceStub.getOfferLettersById('SMU1002','1')
      .pipe(map(response => {
        component.offerLetter = response.json().data;
      }))
    tick();
  }));
  ...
  // it('#generateOfferLetter - if condition', () => {
  ...
  it('#showImage - 404 response', () => {
    spyOn(component,'showImage').withArgs(nulldata).and.returnValues(null);
    // expect(component.imagePopSuccess).toBe(false);
    // expect(component.imagePop).toBe(true);
    // expect(component.error).toEqual("Please provide Valid information");
  })
