...
describe('UserListComponent', () => {
  ...
  it('#addRolestoUserPop', fakeAsync(() => {
    component.addRolestoUserPop('1');
    tick();
    expect(component.isShowPopup).toBe(true);
  }));

  it('#addRoletoUser', fakeAsync(() => {
    component.addRoletoUser();
    tick();
    expect(component.rolesUpdateStatus).toBe(true);
  }));

  it('#offerLetter', fakeAsync(() => {
    component.offerLetter('1');
    tick();
    expect(component.isShowPopup).toBe(true);
  }));
  ...
  it('#search', fakeAsync(() => {
    ...
    tick();
    expect(component.filterData.length).toEqual(component.users.length);
