  ...
  ngOnInit() {
    ...
    this.authenticationService.getTheme()
      .subscribe(response => {
        ...
        this.CompanyLogo = result.data.siteLogo;
      });
                              ...
                              this.superService.getCompanyInfo()
                                .subscribe(response => {
                                  ...
                                  if (response.statusCode.code == '200') {
                                    ...
                                    this.result = response.decode;
                                    this.companyData = {
                                      clientName:this.result.companyName,
                                      clientId:this.result.clientId,
                                      clientLogo:this.CompanyLogo,
                                      clientUrl:this.result.api_url,
                                      email:this.result.email,
                                      website:this.result.website,
                                      frontendUrl:this.result.frontend_url,
                                    };
                                    this.superService.addCompanyInfoToJobportal(this.companyData)
                                    .subscribe(response => {
                                      console.log(response);
                                      if (response.statusCode.code == '200') {
                                        this.router.navigate(['/authorization/active-user']);
                                      }
                                      else {
                                        this.message5 = 'Failed to add  Company Information to Tedpros Jobportal.';
                                      }
                                    });
