...
import { EditWorkWeekComponent } from './edit-work-week.component';
...
describe('EditWorkWeekComponent', () => {
  let component: EditWorkWeekComponent;
  let fixture: ComponentFixture<EditWorkWeekComponent>;
  const userServiceStub = new UserServiceStub();
  const locationStub = new LocationStub();
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      declarations: [ EditWorkWeekComponent ],
      providers: [
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(EditWorkWeekComponent);
  ...
  it('#ngOnInit',fakeAsync(()=>{
    component.ngOnInit();
    tick();
    userServiceStub.getWorkWeek().subscribe(response => {
      component.weeks = response.json().data;
      console.log(response.json().data);
    });

    userServiceStub.getWorkDay().subscribe(res => {
      component.weekData = res.json().data;
    });

    userServiceStub.getCountries()
      .subscribe(response => {
        component.countries = response.json().data;
        // this.loading = false;
      });
}));
...
it("#editWork",fakeAsync(()=>{
  ...
  component.editWork();
  tick();
  expect(component.editdata.statusCode.code).toEqual('200');
