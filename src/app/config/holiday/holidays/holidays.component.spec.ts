...
describe('HolidaysComponent', () => {
  ...
  it('#ngOnInit',fakeAsync(()=>{
    component.ngOnInit();
    tick();
    userServiceStub.getHolidaysList().pipe(map(res => {
      component.holidayList = res.json().data;
    }));

  }));
 it("#getHolidayById",fakeAsync(()=>{
        component.showJobDetails('21');
        tick();
        expect(component.isShowPopup).toBe(true);
        expect(component.holidayInfo.id).toEqual("21");
  ...
  it("#search",fakeAsync(()=>{
    ...
    tick();
    expect(component.filterData.length).toEqual(component.holidays.length);
