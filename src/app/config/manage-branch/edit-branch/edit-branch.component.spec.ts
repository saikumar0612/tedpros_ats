...
describe('EditBranchComponent', () => {
  ...
  it('#ngOnInit', fakeAsync(() => {
    component.ngOnInit();
    tick();
    userServiceStub.getBranch('3').pipe(map(res => {
      component.branch = res.json().data;

      component.getStates(component.branch.registeredAddress.country.id, 1);
      component.getCitys(component.branch.registeredAddress.state.id, 1);
    }));

  }));
  it('#getStates', fakeAsync(() => {
    component.loading = true;
    component.getStates('231', 1);
    tick();
    expect(component.loading).toBe(false);

  }));

  it('#getCitys', fakeAsync(() => {
    component.loading = true;
    component.getCitys('3977', 1);
    tick();
    expect(component.loading).toBe(false);
  ...
  it('#getAddress', fakeAsync(() => {
    component.branch.registeredAddress.zipcode = '25259';
