...
describe('SettingsComponent', () => {
  ...
  it('#ngOnInit', fakeAsync(()=>{
    component.ngOnInit();
    tick();
    expect(component.candidateId).toEqual(component.candidateUser.data.id);
  }));

  it('#getCoverLetter', fakeAsync(()=>{
    component.getCoverLetter();
    tick();
    // expect(component.)
  }));

  it('#addCoverLetters', fakeAsync(()=>{
    component.formDisplay = false;
    component.addCoverLetters();
    tick();
    expect(component.formDisplay).toBe(true);
  }));

  it('#closeOfferLetter', fakeAsync(()=>{
    component.formDisplay = true;
    const letterFrm:NgForm =new NgForm([],[]);
    component.closeOfferLetter(letterFrm);
    tick();
    expect(component.formDisplay).toBe(false);
  }));

  it('#addCoverletter', fakeAsync(()=>{
    const letterFrm:NgForm = new NgForm([], []);
    component.addCoverletter(letterFrm);
    tick();
    expect(component.data.statusCode.code).toEqual('200');
  }));

  it('#addInfo', fakeAsync(()=>{
    component.addInfo();
    tick();
    expect(component.result.statusCode.code).toEqual('200');
  }));

  it('#closeSettingsPopup', fakeAsync(()=>{
    component.settingsPopup = true;
    component.closeSettingsPopup();
    tick();
    expect(component.settingsPopup).toBe(false);
  }));
  ...
  it('#openPopup', fakeAsync(()=>{
    component.coverLetterDetails = false;
    component.openPopup('1');
    tick();
    expect(component.coverLetterDetails).toBe(true);
  }));

  it('#closePopup', fakeAsync(()=>{
    component.coverLetterDetails = true;
    component.closePopup();
    tick();
    expect(component.coverLetterDetails).toBe(false);
  }));
  ...
  it('#password', fakeAsync(()=>{
    component.passwordPopup = false;
    component.password();
    tick();
    expect(component.passwordPopup).toBe(true);
  }));

  it('#closePasswordPopup', fakeAsync(()=>{
    const passwordForm:NgForm = new NgForm([],[]);
    component.passwordPopup = true;
    component.closePasswordPopup(passwordForm);
    tick();
    expect(component.passwordPopup).toBe(false);
  }));

  it('#changePassword', fakeAsync(()=>{
    const passwordForm:NgForm = new NgForm([],[]);
    component.changePassword(passwordForm);
    tick();
    expect(component.response.statusCode.code).toBe('200');
  }));

  it('#closeEmailPopup', fakeAsync(()=>{
    const emailForm:NgForm = new NgForm([],[]);
    component.emailPopup = true;
    component.closeEmailPopup(emailForm);
    tick();
    expect(component.emailPopup).toBe(false);
  }));

  it('#closechangePasswordPopup', fakeAsync(()=>{
    component.changePasswordPopup = true;
    component.closechangePasswordPopup();
    tick();
    expect(component.changePasswordPopup).toBe(false);
  }));

  it('#email', fakeAsync(()=>{
    component.emailPopup = false;
    component.email();
    tick();
    expect(component.emailPopup).toBe(true);
  }));

  it('#changeEmail', fakeAsync(()=>{
    const emailForm:NgForm = new NgForm([],[]);
    component.changeEmail(emailForm);
    tick();
    expect(component.responce.statusCode.code).toEqual('200');
  }));

  it('#closeEmailErrorPopup', fakeAsync(()=>{
    component.changeEmailErrorPopup = true;
    component.closeEmailErrorPopup();
    tick();
    expect(component.changeEmailErrorPopup).toBe(false);
  }));
  ...
  // it('#closechangeMailPopup', fakeAsync(()=>{
  //   component.closechangeMailPopup();
  //   tick();
  //   // expect(component.logout)
  // }));
  ...
  //   component.checkMail();
  //   tick();
  //   expect(component.result.statusCode.code).toEqual('409');
  //   expect(component.emailError).toEqual('This email is already registered');
  // }));

  it('#addDocuments', fakeAsync(()=>{
    component.addDoc = false;
    component.addDocuments();
    tick();
    expect(component.addDoc).toBe(true);
  }));

  it('#closeDocuments', fakeAsync(()=>{
    const documentFrm:NgForm = new NgForm([],[]);
    component.addDoc = true;
    component.closeDocuments(documentFrm);
    tick();
    expect(component.addDoc).toBe(false);
  }));

  it('#close', fakeAsync(()=>{
    const documentFrm:NgForm = new NgForm([],[]);
    component.addDoc =true;
    component.close(documentFrm);
    tick();
    expect(component.addDoc).toBe(false);
  }));

  it('#getDocuments', fakeAsync(()=>{
  ...
  it('#deleteDoc', fakeAsync(()=>{
    ...
    tick();
    expect(component.deletedocData.statusCode.code).toBe('200');
  }));
});
