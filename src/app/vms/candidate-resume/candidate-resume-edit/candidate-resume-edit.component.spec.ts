...
import { CandidateResumeEditComponent } from './candidate-resume-edit.component';
...
describe('CandidateResumeEditComponent', () => {
  let component: CandidateResumeEditComponent;
  let fixture: ComponentFixture<CandidateResumeEditComponent>;
  const VmsService = new VmsServiceStub();
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      declarations: [CandidateResumeEditComponent],
      providers: [
        { provide: VmsCandidateService, useValue: VmsService },
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(CandidateResumeEditComponent);
...
//    component.candidateUser = localStorage.getItem('candidateUserData');
  });
  ...
  it('#dateOfBirthValidation', fakeAsync(()=>{
    ...
    tick();
    // component.age =  new Date('Fri Jul 26 2019 12:00:00 GMT+0530 (India Standard Time)');
    // expect(component.doberror).toEqual('Candidates Below the Age 14 are not eligible');
    // expect(component.age).toEqual('14');
  }));

  it('#getSkills', fakeAsync(()=>{
    component.getSkills();
    tick();
  ...
  it('#dateValidate', fakeAsync(()=>{
    ...
    component.experienceData = {
      experience: {
        startDate: new Date('Wed Oct 09 2019 17:32:49 GMT+0530 (India Standard Time)'),
        endDate: new Date('Thu Oct 10 2019 17:32:49 GMT+0530 (India Standard Time)')
      }
    };
  ...
  it('#ngOnInit', fakeAsync(()=>{
    component.ngOnInit();
    tick();
    expect(component.id).toEqual(component.personalData.id);
  ...
  it('#getAddress', fakeAsync(()=>{
    ...
    tick();
    expect(component.addressData.statusCode.code).toEqual('200');
  }));


  it('#addnewskill', fakeAsync(()=>{
    component.skillpopup = false;
    component.addnewskill();
    tick();
    expect(component.skillpopup).toBe(true);
  }));

  it('#closeskillPopup', fakeAsync(()=>{
    const addindFrm:NgForm = new NgForm([],[]);
    component.skillpopup = true;
    component.closeskillPopup(addindFrm);
    tick();
    expect(component.skillpopup).toBe(false);
  }));

  it('#addskills', fakeAsync(()=>{
    const addindFrm:NgForm = new NgForm([],[]);
    component.addskills('skillType', addindFrm);
    tick();
  ...
  it('#addSkill', fakeAsync(()=>{
    ...
    component.addSkill('skillName', 'experience', 'month', 'year', 'comment', skillFrm);
    tick();
    expect(component.skillError).toEqual('Duplicate skill data cannot be added');
  }));

  it('#addEducation', fakeAsync(()=>{
    const editEducations:NgForm = new NgForm([], []);
    component.addEducation('instituteName', 'qualification', 'specialization', 'gpaScore', editEducations);
    tick();
  ...
  it('#addExperiences', fakeAsync(()=>{
    ...
    component.addExperiences('employer', 'jobTitle', 'description', 'supervisorName', 'supervisorPhone', 'responsibilities', 'techSkills', expForm);
    tick();
    expect(component.experienceError).toEqual('');
  }));

  it('#removeEducation', fakeAsync(()=>{
    component.removeEducation({instituteName:'instituteName',qualification:'qualification',specialization:'specialization',passingYear:'passingYear',gpaScore:'gpaScore'});
    tick();
    expect(component.educations).toEqual([]);
  }));

  it('#removeExperience', fakeAsync(()=>{
    component.removeExperience({employer:'employer',jobTitle:'jobTitle',description:'description',supervisorName:'supervisorName',supervisorPhone:'supervisorPhone',responsibilities:'responsibilities',techSkills:'techSkills'});
    tick();
    expect(component.experiences).toEqual([]);
  }));

  it('#removeSkill', fakeAsync(()=>{
    component.removeSkill({skillName:'skillName', name:'name', experience:'experience', month:'month',year: 'year', comment:'comment'});
    tick();
  ...
  it('#editCandidate', fakeAsync(()=>{
    ...
    expect(component.message).toEqual('personal data updated successfully');
    expect(component.isShowPopup1).toBe(true);
  }));

  it('#closePopup1', fakeAsync(()=>{
    component.consentPopup = true;
    component.isShowPopup1 = true;
    component.closePopup1();
    tick();
    expect(component.consentPopup).toBe(false);
    expect(component.isShowPopup1).toBe(false);
    expect(component.error).toBe('');
    expect(component.message).toBe('');
  }));

  it('#consentClose', fakeAsync(()=>{
    component.consentPopup = true;
    component.consentClose();
    tick();
    expect(component.consentPopup).toBe(false)
  }));

  it('#editConsentData', fakeAsync(()=>{
    component.editConsentData();
    tick();
    expect(component.consent.statusCode.code).toEqual('200');
  }));
  ...
  it('#getFiles', fakeAsync(()=>{
    component.getFiles(event);
    tick();
    // expect(component.files).toEqual(event.target.files);
  }));

  it('#deleteResume', fakeAsync(()=>{
    component.deleteResume();
    tick();
    expect(component.deleteData.statusCode.code).toEqual('200');
  }));

  // it('#getResume', fakeAsync(()=>{
  //   component.getResume();
  //   tick();
  //   expect(component.resume).toEqual(component.resumeData.document);
  // }));

  it('#closeResumePopup', fakeAsync(()=>{
    component.fileName = true;
    component.closeResumePopup();
    tick();
    expect(component.fileName).toBe(false);
    expect(component.fileData.resume).toEqual('');
  }));


  it('#closeresumePopup1', fakeAsync(()=>{
    component.resumePopup = true;
    component.closeresumePopup1();
    tick();
    expect(component.resumePopup).toBe(false);
    expect(component.error).toEqual('');
  }));


  it('#editCanExperience', fakeAsync(()=>{
    const experience  =[{
    ...
    component.editCanExperience(experience);
    tick();
    expect(component.indexofexperience).toEqual(-1);
  }));

  it('#editCanEduaction', fakeAsync(()=>{
    const education = [{
        instituteName:'instituteName',qualification:'qualification',specialization:'specialization',gpaScore:'gpaScore',isCurrent:'isCurrent'
    }];
    component.editCanEduaction(education);
    tick();
    expect(component.indexofEducation).toEqual(-1);
  }));

  it('#editCanSkill', fakeAsync(()=>{
    component.skills = [{
  ...
  it('#editCandidateExperience', fakeAsync(()=>{
    ...
    component.experiences = [{
      employer: 'employer',jobTitle: 'jobTitle',description: 'description',startDate: 'startDate',endDate: 'endDate',supervisorName: 'supervisorName',supervisorPhone: 'supervisorPhone',isCurrent: 'expCurrent',responsibilities: 'responsibilities',techSkills: 'techSkills'
    }];
    component.editCandidateExperience();
    tick();
    expect(component.responseData.statusCode.code).toEqual('200');
    expect(component.message).toEqual('Experience data updated successfully');
    expect(component.isShowPopup1).toBe(true);
  }));
  ...
  it('#editCandidateEducation', fakeAsync(()=>{
    ...
    tick();
    expect(component.responseData.statusCode.code).toEqual('200');
    expect(component.message).toEqual('Education data updated successfully');
    expect(component.isShowPopup1).toBe(true);
  }));
  ...
  it('#editCandidateSkills', fakeAsync(()=>{
    ...
    expect(component.isShowPopup1).toBe(true);
    component.skillData = [{
       skillName:'skillName', experience:'experience',lastUsed:'lastUsed',comment:'comment',month:'month',year:'year' }];
    component.editCandidateSkills();
    tick();
    expect(component.responseData.statusCode.code).toEqual('200');
    expect(component.message).toEqual('Skills data updated successfully');
    expect(component.isShowPopup1).toBe(true);
  }));
  ...
  it('#closePreviewPopup', fakeAsync(()=>{
    component.closePreviewPopup();
    tick();
    expect(component.preview).toBe(true);
  }));

  it('#closeconsentPopup', fakeAsync(()=>{
    component.isShowSummery = true;
    component.consentPopup = true;
    component.closeconsentPopup();
    tick();
    expect(component.isShowSummery).toBe(false);
    expect(component.consentPopup).toBe(false);
  }));
  ...
  it('#uploadPic', fakeAsync(()=>{
    component.uploadPic();
    tick();
    expect(component.uploadData.statusCode.code).toEqual('200');
  }));
