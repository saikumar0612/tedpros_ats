...
describe('MyTimesheetsComponent', () => {
  ...
  it('#ngOnInit', () => {
    const today = new Date('Mon Sep 23 2019 00:00:00 GMT+0530 (India Standard Time)');
    const end = new Date((new Date(today)).getTime() + (60 * 60 * 24 * 1000 * 6));
    component.todayDate = today;
    component.ngOnInit();
    tick();
    expect(component.timesheetData.dateRange).toEqual(today);
    expect(component.timesheetData.endDate).toEqual(end);
  });

  it('#cangedate1', () => {
    const today = new Date('Mon Sep 23 2019 00:00:00 GMT+0530 (India Standard Time)');
    const end = new Date((new Date(today)).getTime() + (60 * 60 * 24 * 1000 * 6));
    component.timesheetData.dateRange = today;
    component.cangedate1();
    tick();
    expect(component.timesheetData.dateRange).toEqual(today);
    expect(component.timesheetData.endDate).toEqual(end);
  });
  ...
  it('#closePopup', () => {
    spyOn(component, 'closePopup1');
    component.closePopup1();
    tick();
    expect(component.closePopup1).toHaveBeenCalled();
  });
